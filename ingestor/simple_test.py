#!/usr/bin/env python3
"""
Simple test script for the news ingestor (without database)
"""
import logging
import sys
...
def test_feed_reading():
    ...
    # Test with Yahoo Finance RSS feed
    test_url = "https://feeds.finance.yahoo.com/rss/2.0/headline"
    logger.info(f"Testing with URL: {test_url}")
    
    feed = feed_reader.fetch_feed(test_url)
    ...
    if feed and feed.entries:
        ...
        for i, entry in enumerate(entries):
            ...
            post_data = content_processor.process_feed_item(entry, source_config)
            ...
            if post_data:
                ...
                logger.info(f"  - Source URL: {post_data['source_url']}")
            else:
...
def main():
    """Run all tests"""
    logger.info("Starting simple ingestor tests...")
